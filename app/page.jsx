...
export default function Home() {
  ...
  useEffect(() => {
    if (selectedChat) {
      ...
      setMessages([]) // Clear messages when selectedChat is null
    }
  }, [selectedChat])
  ...
  useEffect(() => {
    if (!user) {
      setMessages([]) // Force clear messages when user logs out
    }
  }, [user])
          ...
          {messages.length === 0 ? (
            <>
              <div className="flex items-center gap-1">
                <Image src={assets.charlesgptlogo} alt="logo" className="h-16" />
                <p className="text-2xl font-bold">Hi, I'm CharlesGPT</p>
              </div>
              <p className="text-sm mt-2 mb-3.5">How can I help you today?</p>
            </>
          ) :
            (
                  ...
                  isLoading && (
                    <div className="flex gap-4 max-w-3xl w-full py-3">
                      <Image className="h-9 w-9 p-1 border border-white/15 rounded-full"
                        src={assets.charlesgptlogo} alt="Logo" />
