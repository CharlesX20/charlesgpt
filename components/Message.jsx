...
const Message = ({role, content}) => {
  ...
  return (
    <div className='flex flex-col items-center w-full max-w-3xl text-sm'>
        <div className={`flex flex-col w-full mb-8 ${role === 'user' && 'items-end mt-6'}`}>
        <div className={`group relative flex max-w-2xl py-3 rounded-xl ${role === 'user' ? 'bg-primary px-5' : 'gap-3'}`}>
            ...
            </div>
            {
                ...
                (
                    <>
                    <Image src={assets.charlesgptlogo} alt='' className='h-9 w-9 p-1 border border-white/15 rounded-full'/>
